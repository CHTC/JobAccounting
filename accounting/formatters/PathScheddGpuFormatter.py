...
class PathScheddGpuFormatter(BaseFormatter):
    ...
    def get_table_title(self, table_file, report_period, start_ts, end_ts):
        ...
        if report_period in ["daily"]:
            ...
            title_str = f"PATh Facility GPU per {info['agg'].rstrip('s')} usage for jobs completed on <strong>{start_date}</strong>"
        elif report_period in ["weekly", "monthly"]:
            end = datetime.fromtimestamp(end_ts)
            start_date = start.strftime("%Y-%m-%d")
            end_date = end.strftime("%Y-%m-%d")
            title_str = f"PATh Facility GPU per {info['agg'].rstrip('s')} usage for jobs completed from <strong>{start_date} to {end_date}</strong>"
        else:
            end = datetime.fromtimestamp(end_ts)
    ...
    def get_subject(self, report_period, start_ts, end_ts, **kwargs):
        ...
        if report_period in ["daily", "weekly", "monthly"]:
            start_date = start.strftime("%Y-%m-%d")
            subject_str = f"PATh Facility AP GPU {report_period.capitalize()} Usage Report {start_date}"
        else:
            end = datetime.fromtimestamp(end_ts)
            start_date = start.strftime("%Y-%m-%d %H:%M:%S")
            end_date = end.strftime("%Y-%m-%d %H:%M:%S")
            subject_str = f"PATh Facility AP GPU Usage Report {start_date} to {end_date}"
        return subject_str
    ...
    def get_legend(self):
        ...
        custom_items["Good GPU Hours"]   = "Total GPU hours for execution attempts that ran to completion"
        ...
        custom_items["GPU Hours / Bad Exec Att"] = "Average GPU Hours used in a non-final execution attempt"
